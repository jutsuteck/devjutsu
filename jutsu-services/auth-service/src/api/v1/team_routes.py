...
from src.schemas.v1.team_schema import (
    TeamCreateSchema,
    TeamReadSchema,
    TeamUpdateSchema
)
...
async def create_team(
    ...
    return await service.create_team(team_create_schema.model_dump())


@router.get('/{team_id}', response_model=TeamReadSchema)
async def get_team_by_id(
        team_id: str, service:
        TeamService = Depends(TeamService)):
...
async def update_team(
        ...
        service: TeamService = Depends(TeamService)):
    return await service.update_team(team_id, team_update_schema.model_dump())
