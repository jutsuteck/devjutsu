...
from src.models.v1.member import Role
...
class RoleService:
    ...
    async def filter_by_member(self, member_id: str) -> List[Role]:
        ...
        if not member_roles:
            raise HTTPException(
                status_code=404, detail=f"No roles found for member with id {member_id}")

        return member_roles

    async def get_role_or_404(self, role_id: str) -> Role:
    ...
    async def update_role(self, role_id: str, role_data: dict) -> Role:
        await self.get_role_or_404(role_id)
